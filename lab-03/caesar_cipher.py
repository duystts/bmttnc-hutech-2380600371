...
class MyApp(QMainWindow):
    ...
    def validate_inputs(self):
        ...
        if not plain_text:
            ...
            plain_valid = False
        elif not plain_text.isalpha():
            self.lbl_plain_error.setText("Chỉ chấp nhận chữ cái (A-Z). Không chứa khoảng trắng, số hoặc ký tự đặc biệt.")
            self.ui.txt_plain_text.setStyleSheet("border: 1px solid red;")
            plain_valid = False
        else:
            self.lbl_plain_error.setText("")
            self.ui.txt_plain_text.setStyleSheet("")
        ...
        if not key_text:
            ...
            try:
                key = int(key_text)
                if key < 1 or key > 25:
                    self.lbl_key_error.setText("Key phải là số nguyên từ 1 đến 25.")
        ...
        if not cipher_text:
            ...
            self.lbl_cipher_error.setText("Chỉ chấp nhận chữ cái (A-Z). Không chứa khoảng trắng, số hoặc ký tự đặc biệt.")
            self.ui.txt_cipher_text.setStyleSheet("border: 1px solid red;")
            cipher_valid = False
        else:
            self.lbl_cipher_error.setText("")
            self.ui.txt_cipher_text.setStyleSheet("")
    ...
    def call_api_encrypt(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Temporarily block signals to avoid triggering validation loops
